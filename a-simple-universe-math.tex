...
However, as argued above, with probability $1$, our hypothetical intelligent
beings would not be able to make predictions for a significant part of
the universe because they would have no idea about how their universe works,
not because they don't know its state precisely enough.
\svn{reformulat}
...
a given designer, it is reasonable
\svn{Rephase. Maybe \ghilimele{acceptable}?}
to think that their set is countable,
and, if so, that we have a positive probability for each of them.

Extrapolating from what we know about ourselves, although that may
not give us much, it is still reasonable to believe that
\svn{finite?}
rational beings,
...
\begin{axiom}\label{unitycovering}
  ...
  having generalized
  \svn{define generalized rational. Check that it's needed, or we can use
       $q\in Q$}
...
\begin{axiom}\label{uncountable}
  ...
  \svn{Except for the $\reale$ cardinality}
\end{axiom}
...
we get a set of countable universe descriptions which we will
