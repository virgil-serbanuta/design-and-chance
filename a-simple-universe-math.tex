...
For a countable set of functions we can do the following: take the
first function, take an infinite subset of hypercubes $A1$ that leaves out
an infinite subset of hypercubes $B1$, for the second function, take an
infinite subset of hypercubes $A2\subset B1$ that leaves out an infinite
subset $B2$, and so on. Then we can flip subsets of $A1 \times A2 \times \dots$.

This means that we need an uncountable set of functions. With the continuum
hypothesis, this means $\reale$.

Does an infinite set of functions of cardinality less than $\reale$ leave out
at least one description? We have a function from descriptions to sets of
functions that get them correctly. Each pair of functions in such a set has a
description that differentiates them. So, given a function, we can take a subset
of descriptions of cardinality less than $\reale$ that separates it from
everything else. This means that we can have a set of descriptions of the same
cardinality that separates between all functions. Also, for any countable subset
we can find $\reale$ functions that are left out. ****

\svn{
  Prove that we need countable mappings and $\reale$ descriptions.
}
\svn{
  This works only with some kinds of statistical modeling because
  we need to fulfill p% of predictions in all hypercubes, we can't,
  e.g., say that we model p% of space with full precision.
}
