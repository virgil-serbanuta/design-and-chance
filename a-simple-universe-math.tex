...
In order for this paper's argument to work, one would actually need a non-zero
probability that the Designer wanted the universe to have intelligent life.

This is a good objection for this approach, and the main question is whether
it is reasonable to assign a non-zero subjective probability to the hypothesis
that a Designer would want to create human-like beings.
If we can't, then this argument can't explain that the universe around us
seems to be consistent, it just points out how extremely odd is to observe it.

If, on the other hand, we take it as a brute fact that the universe has a
finite description (which is not the only possible option - we could just
live in a bubble of consistency whose probability becomes inimaginably
smaller with each passing second), then it's somewhat more reasonable to
observe this consistency, but it also means that we are in the right setting
for the argument presented in ???Swinburne??, which shows that this consistency
...
purpose has the same cardinality as $\reale$.
...
there were many similar purposes that were discarded in favor of the chosen
one.
...
purpose of this \paper{}, it's probably good enough to show that, from our
point of view, we should assign a non-zero probability to both.

First, let us note that, for real numbers, the similarity of two numbers
usually means the inverse of the distance between them.
So, if we are given a
non-zero distance (i.e. a non-infinite similarity) between real numbers,
and we want to pick a subset of real numbers that have at least that distance
between them, then that subset is at most countable.

It is not obvious that, although their sets have the same cardinality,
similarity between purposes works in the same way as similarity between
real numbers, but I think that the above shows that there's a non-zero chance
that, by discarding purposes that are too similar to a chosen one, the set
of chosen purposes would be countable.

Given a countable set of options, then we can (and should) assign by default
a non-zero probability to all options.

Second, we can make an analogy with ourselves, and, since we do find it
interesting to create intelligence, a Designer of worlds might also find it
interesting.
How likely is it that this is a good analogy?
It's hard to say, but, again,
any small (but non-zero) probability that this analogy works would be enough,
and I think that the analogy is good enough for that.

Also, a purpose that includes creating intelligent beings seems fairly different
from a purpose that doesn't, so, if we would like to choose a maximal set of
purposes such that no two are too similar, then this set would contain at
least one purpose which involves creating intelligent beings.
This is another reason why I think that it's reasonable to assign a non-zero
probability to the hypothesis that a Designer of worlds may design a world for
intelligent beings.

TODO: I may also start with all purposes that can be expressed with a finite
number of words, then I would take all combinations (finite or infinite) of
them, then show that the distance on these is similar enough to the distance
on real numbers, and the finite ones are similar enough with $\rationale$.
Then, would one be able to avoid a purpose of creating intelligent beings?
Maybe, but another approach would be to say that each infinite combination
is approximated arbitrarily well by finite ones, so it's probably good enough
to work only with these.

So, if we have a non-zero probability that the set of purposes is countable,
a non-zero probability that one of them involves creating intelligent beings,
and each of the purposes in the set has a default non-zero probability,
then the overall probability of having a universe created for intelligent
beings is non-zero.
...
As argued in
?Swinburne??, it's very likely that this universe is, indeed, consistent on
a large scale. But if there is a non-zero probability of observing
