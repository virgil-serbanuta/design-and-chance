...
If our universe is not designed, then there is a level of approximation $L$ and a rational number $r$ such that the following axioms hold. If it is designed, then axiom \ref{noreason} does not hold.\svn{Explain why. Well, there is nothing to explain, these axioms are true in all cases. Think more about this.}
...
\item \label{noreason} If $A$ has the same cardinality as $\reale$ and one observes an element $a\in A$, and there is absolutely no reason for $a$ being observed over any other element, then $a$ is generic.
  ...
  \end{itemize}
\item \label{ourplausible} $P(R_{ours} \mid Designer) > 0$ and $P(Designer)>0$.\svn{The introduction for all axioms, \ghilimele{If our universe is not designed ...}, does not apply for this axiom. }
\end {enumerate}
