...
that if no two actual purposes are too similar, then their set
is at most countable. Arguably, that chance is fairly high.
...
For any given similarity threshold
(todo: this is the first time I mention thresholds),
there are multiple ways in which we can split the purposes into disjoint
buckets of similar purposes. As mentioned above, there is a non-zero
probability that we will get a set of buckets that is at most countable,
so let us examine this case in more detail.

If this set is countable, then for any bucket there is a non-zero chance that
the Designer will pick a purpose from it. If we make the similarity threshold
high enough, then there's a good chance that there will be a bucket containing
only purposes involving intelligent beings. If that's the case, then we
have a non-zero probability that the set of buckets is countable, and a
non-zero chance of choosing the bucket with intelligent beings purposes,
