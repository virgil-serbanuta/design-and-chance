...
This result should probably be good enough, and other papers looking at what
we can believe about our universe, e.g. \cite{Calude2013}, seem to agree that,
when we are observing real numbers, they are generic. If you agree with
...
In general, let us let us assume that for some ordinals $\delta$ with
$0 < \delta \le \alpha$ there is a probability distribution
$p_\delta$ over $\descriptions$, telling us
the probability that $y$ is a discontinuity
for all lower ordinal probabilities, i.e. for all
$p_\gamma$ with $\gamma < \delta$.
We will take $p_0$ to be the
...
However, if the probability of having a large consistent time-space region
is positive, then
...
would want rational beings\svn{A designer could want a consistent universe
...
the hypothesis that a Designer exists.
...
works is enough to give the design hypothesis a larger probability.
...
designed by checking if its production involves a mind or it is only
...
The current argument does not try to explain how design looks like, and then
check if the natural laws follow that, instead, it attempts to look at how
non-designed things should look like: rather chaotic, and if there is some
order, we should probably find it outside of our universe, perhaps in a
meta-universe, perhaps it's due to a Desiger.
In a similar way, we explain some of the order we find in our universe at
a higher lever, either in the laws of the universe, or as being designed.

Of course, the current argument does not show that the non-design
hypothesis for our universe
is logically impossible, it just shows that, under the non-design hypothesis,
our universe is much less plausible. Even for the people that are skeptical
about the analogy used above, it shows that the design
...
\svn{Do I need the preceeding $4$ paragraphs,
from \ghilimele{There is a second reason}?
Does it help?}
