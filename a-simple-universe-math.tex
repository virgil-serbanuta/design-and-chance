...
\begin{axiom}\label{finiteneighborhood}
  There is a large time-space region of our universe which includes our solar
  system, and which has a finite approximate description.
\end{axiom}
...
\svn{
    ...
    See the "Not enough descriptions" section.
}
...
are distinct for $L$\svn{not defined!}. By splitting an infinite timespace
...
which we will denote by $\descriptions$. This means that the set of
description has at least the cardinality of $\reale$.
...
lowest description solves the issue, make an explicit comment about it.}
...
Let us think what it means that a meta-universe does not exist. If there is
nothing influencing what universes exist and what universes do not exist, we
expect only two outcomes: either nothing exists, or every conceivable universe
exists. We can reject the former since we observe such a universe.

If, say, only one universe exists, then we can take the set containing it
as a meta-universe with a rule specifying that only this universe exists.
...
and for the other universes to not exist, then there is no meta-universe,
...
generic.
\svn{+quote. What did I mean by \ghilimele{quote}?}
...
Another possible objection is that almost all of the universes used
in this paper can't really exist.
In order to avoid this objection, the current argument works with
conceivable universes, not with possible or existing ones.
...
The cardinal numbers remain the same when restricting the sets of descriptions
to sets that include our universe's description \svn{Huh? This sounds stupid.
After re-reading thrice I get what I wanted to say, but I don't think it's
helpful.}
and restricting the claims to claims which, among others, say that our
universe's description is possible. Unlike in the main topic discussed in this
paper, this time it's clear that only one of these claims can be true. If
we try to model this in terms of probability distributions, we have a
distribution which assigns $1$ to one of these possiblities, and $0$ to
the set containing everything else.

However, we do not know the set of possible descriptions, and we have no hint
...
working with a set of cardinal $\reale$, the only reasonable
probability that we can assign to any concrete claim is $0$.

Even more, the set of all sets of descriptions of cardinality less than $\reale$
has a cardinality which is less than $2^\reale$, which means that it's
reasonable to assume that it has probability $0$. Therefore, with probability
$1$, the set of possible descriptions has the same cardinality as $\reale$.

Also, as above, by grouping similar descriptions in a set and applying the
same reasoning to sets of groupings instead of sets of descriptions,
with probability $1$, the set of possible descriptions has $\reale$
descriptions, which are very different, which is enough for this paper.
\svn{missing contexts}
\svn{Do I need the preceeding $4$ paragraphs, from "There is a second reason"?
Does it help?}
...
To make things simpler, let us assume that $n=1$. It should be obvious that
the reminder of this example can be generalized to any $n>0$.
...
We can represent the past of a hypercube $x\in\intregy$ by a pair $(x, f)$
the function
...
of some hypercubes to their future.
And any universe description must have a way of making hypercube type
predictions.

Note that any such way of making predictions can be identified with a function
from the past of hypercubes to the type of the next hypercube.
...
The past of hypercube $0$ is perfectly identical, so, in order to predict
the hypercube with index $0$, we need two distinct prediction functions.
...
predictions being "true with probability $p>0$".
However, it's valid to ask what happens if we want our predictions to be true
in only a fraction $p$ of spacetime, with probability $p$.

Anyone who wants to talk about \ghilimele{a fraction $p$ of spacetime} needs a
measure function over the entire spacetime.
But, over an infinite universe, for any such measure and any fraction $p<1$,
one can pick a finite amount of timespace which cover that fraction $p$ of the
universe.
If we look at a finite chunk of the universe, then, obviously, it's more regular
than the others, but that does not tell us much about the complexity of the
entire universe.

But let us assume that one has a way of identifying fractions $p$ of universes
that works well enough intuitively.
As an example, given a fraction $p$, one may
divide the spacetime in finite chunks of same size, and ask that, within each,
a fraction close enough to $p$ of hypercubes are part of a specific set,
e.g. part of the set where predictions work.

\svn{
  What are the \ghilimele{hypercubes} above? A fraction of space does not
  mean that we can use hypercubes for that. I must make this assumption
  explicit.
}
...
Let us denote by $n$ the number of chunks we get if we split the entire space
in $p\%$ chunks, one of which may be smaller than $p\%$.
As above, we can probably have $2n$ different hypercube types with the level
of approximation $p$.

We are interested in hypercube type assignments that assign the hypercube types
in a way that the universe has a fraction less than $p/2$ for each of them.
We'll further assume that when we divide the spacetime in chunks of the same
size (hypercubes are a good canditate),
we can get at least two assignments for the hypercubes inside those
chunks such that the two assignments do not assign the same value to the
same (as relative to the chunk) hypercube in both.

Now, any description which works in a fraction $p$ of space must make
predictions that, at least sometimes, work in at least two hypercubes.
Since the universe can have at least two completely different universe chunks,
as mentioned above, in order to make predictions, a definition must identify
the chunk type. As in section \ref{sec:fewuniverses}, we must have at least
$\reale$ definitions.

\svn{Send people here for extensions of what $p$ means.}
...
\subsection{No meta-universe}

We need to take a closer look at some particular cases for what can exist and
what can't.
As an example, it may intuitively seem that it is reasonable to think that
all universes exists, and that this specific instance of what can exist has
a non-zero probability, although \ghilimele{all exist} is a peculiar
predicate when considering
what subset of conceivable universes actually exist.

If so, then we can ask ourselves if this entire argument makes sense.
And, assuming that it makes sense, why can't we have a greater than zero
probability that only one universe exists.

First, I am arguing that, if a meta-universe exists, then the argument in this
paper applies, and one's intuition is wrong. But, actually, there is a hidden
assumption in the intuition that it's somewhat likely that all universes exist.
All universes exist if there is nothing constraining what can exist, i.e.
there is no meta-universe.

If this hypothesis is reasonable, then, as argued in section \ref{fdaumu},
we have at most two choices for what can exist, so the predicate
\ghilimele{all exist} is no longer peculiar. This paper does not discuss if
