...
Let us assume (as an axiom) that, for a given level of approximation, we can
find a finite hypercube size $l$ such that there is a finite set of approximate
descriptions $I$ which can't all apply to the same hypercube of that size, and,
for any set of hypercubes whose descriptions include the $I$ set we can't
find a single approximate description that works for all of them.
...
way\footnote{The set of hypercubes is countable, so there is no standard
way of doing an even distribution, but any that looks reasonable would do}.
Then let us assume that this universe has a finite description. But this
description can be used to make predictions with that level of approximation
in all hypercubes, so for any hypercube of size $1$ we can have a finite
approximate description saying, roughly, \ghilimele{This is a hypercube of
size $1$ in a universe with this description}, which contradicts the assumption
above.\svn{This works only with some kinds of statistical modeling.}

Of course, we need to show that this assumption is reasonable. Let us assume
a universe which is similar to ours, but in which each hypercube has different
fundamental constants. It does look then that, by choosing these constants
carefully, we could make almost all predictions that work in one fail in the
other. Even if that's not enough, it looks like we could change not only the
constants, but the universe laws themselves, until they are different enough.
