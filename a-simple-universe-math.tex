...
disregard\footnote{
  ...
  if our universe is designed, which would means that $\our_description$ has a
...
purposes, i.e. wheher is it at most countable,
...
this bucket contains only purposes involving intelligent beings, and a
...
as $\reale$, and, if we reject the continuum hypothesis\footnote{
  The continuum hypothesys states that there is no set whose cardinality is
...
and, by default, we should use continous probability distributions, and
any if we observe any element with probability $0$, it should be generic.
...
TODO: Cite Theorem 31.7 from the same book. Perhaps also page 422. Exemplul 38.1.
