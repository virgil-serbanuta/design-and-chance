...
descriptions, so $\our_description$ is generic (axiom \ref{ax:noprobability}).
...
$p_\beta$ does not exist or $p_\beta(\our_description) = 0$.
...
was made on a probability distribution
where $p_\beta(\our_description)=0$, then $\our_description$ is
generic (axiom \ref{ax:zeroisgeneric}).

If such a $\beta$ does not exist, up to $\alpha$, without including it,
$p_\alpha$ exists and $p_\alpha(\our_description) > 0$.

If $\alpha$ is countable
then, similar to the above reasoning,
$\our_description$ is generic.
...
for all $\beta < \alpha$, $p_\beta$ exists and $p_\beta(\our_description) > 0$.
...
peculiar $\our_description$, which, by default, is prohibitively unlikely for any of them.
