...
state of the universe when making predictions.
...
This is especially visible, e.g., when claiming
...
\begin{axiom}\label{uncountable}
  ...
  \begin{enumerate}
    ...
    \item For all elements $d\in D_L$ there is at least one conceivable
      ...
      \end{enumerate}
    \item If $d$ and $d'$ are descriptions from $D_L$, then $d$ does not work
...
descriptions, so $OURD$ is generic (axiom \ref{ax:noprobability}).
...
For many people, this result may be good enough. This seems to be taken in
other papers looking at our universe, e.g. \cite{Calude2013}. If you agree with
this, please skip to section \ref{sec:peculiarregions}.

However, let us see what happens when this probability distribution is not
continuous.
Let us let us assume that for some ordinals $\beta$ with
...
$p_\gamma$ with $\gamma < \beta$.
Since we need $P_0$ to define the above, we will take it to be, the
...
$p_\beta$ does not exist or $p_\beta(OURD) = 0$.

Before ordinal $\beta$, we made a countable number of observations on various
probability distributions, one for each ordinal/meta-universe. Since one of them
was made on a probability distribution where $p_\beta(OURD)=0$, then $OURD$ is
generic (axiom \ref{ax:zeroisgeneric}).

If such a $\beta$ does not exist, up to $\alpha$, without including it,
$p_\alpha$ exists and $p_\alpha(OURD) > 0$.

If $\alpha$ is countable
then, similar to the above reasoning,
$OURD$ is generic.

This means that, if our universe is not designed, we have two options
that might be reasonable: our universe's description, $OURD$, is
generic, or ($OURD$ is peculiar, $\alpha$ is uncountable and,
for all $\beta < \alpha$, $p_\beta$ exists and $p_\beta(OURD) > 0$.
...
Then, if the probability that a designer would want rational beings is non-zero,
this would make the probability of observing our universe under the design
hypothesis higher than the non-design one, therefore providing an ammount of
support for design that roughly corresponds to the ratio between this
probability and the probability of observing this world without design.
If the same probability would not be vanishingly small, it would provide
...
\end{itemize}
The first two options are valid only if, for a given approximation level,
the set of approximate descriptions finite hypercubes has a lower cardinality
if we require that the universe contains life.
...
want peculiar things, or homogeneity. Also, we can use an analogy with
ourselves in order to figure out a Designer's intent. Then we would have to
choose between two extremely improbable options, and an analogy with
unknown chances of working. If there's the slightest chance that the analogy
works, we should bet on that.
...
As an example, with our current understanding of physical laws, it is logically
possible that a watch's pieces are produced by chance and they come together
by chance to form a watch, or it could happen that the watch materializes
out of thin air due to random quantum fluctuations, but that's way too
implausible, while it's more likely that any watch that we observe was
designed.

In the same way, the current argument does not show that the non-design
hypothesis for our universe
is logically impossible, it just shows that, under the non-design hypothesis,
our universe is much less plausible, or, at least, it shows that the design
hypothesis is the only one which is not provably implausible.
...
of approximation $L$. For each way of assigning these hypercube types,
we get one type of universe.
...
\item The Designer intent analogy makes some sense, our universe is designed,
      and what we expect about its homogeneity is as homogenous as we would
      expect it to be.
